...
;;;
;;; This part of the file provides a basic object system, and 
...
(define (set-instance-name! instance name)
  (set-car! (cddr instance) name))
...
(define (create-instance maker . args)
  (let* ((instance (make-instance))
    ...
    (if (method? (get-method 'NAME instance))
        (set-instance-name! instance (ask instance 'NAME)))
    instance))
...
(define our-clock (create-clock))

;; Get the current time
(define (current-time)
  (ask our-clock 'THE-TIME))

;; Advance the clock some number of ticks
(define (run-clock n)
  (cond ((= n 0) 'DONE)
        (else (ask our-clock 'TICK)
